...
- **AI連携**: Anthropic API (Claude) — 過去見積Excelの解析・パターン抽出、
  見積のたたき台提案

### 画面構成
1. **サインアップ/ログイン** (`/signup`, `/login`) — Supabase Auth(メール+パスワード)
2. **オンボーディング** (`/onboarding`) — 会社基本情報の入力 → 過去見積Excelの
   アップロード → AIが品目・単価パターンを抽出 → 内容を確認して取り込み
   (列の判定に自信が持てない場合は、検出した列見出しを提示し、手動で
   列を指定して再解析できる)
